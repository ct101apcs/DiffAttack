...
def max_entropy_loss(attn_map, eps=1e-10):
    """
...
def diffattack(
    ...
    for _, _ in enumerate(pbar):
        ...
        # "Deceive" Strong Diffusion Model via Maximum Entropy
        # OLD: variance_cross_attn_loss = after_true_label_attention_map.var() * args.cross_attn_loss_weight
        # NEW: Max Entropy Loss - maximizes spatial distribution entropy
        entropy_val = max_entropy_loss(after_true_label_attention_map)
        variance_cross_attn_loss = entropy_val * args.cross_attn_loss_weight
        ...
        if verbose:
            pbar.set_postfix_str(
                ...
                f"entropy_loss: {variance_cross_attn_loss.item():.5f} "
